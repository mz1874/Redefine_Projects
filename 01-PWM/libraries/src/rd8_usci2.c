...
StatusTypeDef USCI2_SPI_Receive(USCI2_HandleInfoDef* USCI2_HANDLE, uint8_t* pData, uint8_t Size, uint32_t Timeout)
{
  ...
  if(USCI2_HANDLE->RxState == USCI2_STATE_READY)
  {
    ...
    if((USXCON0 & USCI2_SPI_MODE_MASTER) != 0)
    {
      return USCI2_SPI_TransmitReceive(USCI2_HANDLE, pData, Size, Timeout);			//跳转到阻塞模式数据收发函数
    }
...
/**********************************************************************************************************
*函数名称:StatusTypeDef USCI2_SPI_TransmitReceive(USCI2_HandleInfoDef* USCI2_HANDLE, uint32_t Timeout)
*函数功能:阻塞模式收发一定量数据
*入口参数:
USCI2_HandleInfoDef*:USCI2_HANDLE:指向包含 SPI0 信息的 USCI2_HandleInfoDef 结构体的指针。
*出口参数:
StatusTypeDef:USCI2 状态
**********************************************************************************************************/
StatusTypeDef USCI2_SPI_TransmitReceive(USCI2_HandleInfoDef* USCI2_HANDLE, uint8_t* pData, uint8_t Size, uint32_t Timeout)
{
  uint32_t TimeoutCnt = 0;
  ...
  if((USCI2_HANDLE->TxState == USCI2_STATE_READY) && (USCI2_HANDLE->RxState == USCI2_STATE_READY))
  {
    ...
    USCI2_ClearFlag(USCI2_SPI_FLAG_SPIF);			//清除中断标志位
    USCI2_HANDLE->pTxBuffPtr.Size_u8 = USCI2_HANDLE->pRxBuffPtr.Size_u8 = pData;			//指向待发送数据的地址
    ...
    while(USCI2_HANDLE->TxXferCount < USCI2_HANDLE->TxXferSize);
    {
      ...
      TimeoutCnt = 0;			//超时计数值清零
      USCI2_HANDLE->TxXferCount++;			//数据量计数值减1
      USCI2_HANDLE->TxXferCount++;			//数据量计数值减1
    }
    if(USCI2_HANDLE->TxXferCount == USCI2_HANDLE->TxXferSize)
    {
...
/**********************************************************************************************************
*函数名称:StatusTypeDef USCI2_SPI_TransmitReceive_IT(USCI2_HandleInfoDef* USCI2_HANDLE, uint8_t* pData, uint8_t Size)
